...
import argparse
import os
from utils import get_data_set, batch_bgr2gray
...
def main():
    ...
    y_pred = unet.create_unet(x)

    y_loss = unet.loss_function(y_true, y_pred)

    # y_pred = tf.argmax(y_pred, axis = 3, name="y_pred")

    optimizer = tf.train.AdamOptimizer(learning_rate=1e-4).minimize(y_loss)
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            for batch_idx in t:
                ...
                feed_dict_tr = {x: batch_train, y_true: batch_label}
